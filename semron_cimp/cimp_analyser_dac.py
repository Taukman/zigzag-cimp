# 3D cell array with ADC grouping and DAC 
...
class CIMP_Analyzer:
    def __init__(self):
        ...
        # --- DAC energy model -------------------------------------------------
        # E_dac = K7 * DAC_res * Vdd^2   (per conversion, per DAC, per Lx step)
        #   K7      = switched-capacitance constant   (Vdd = Vin, DAC_res = Sx)
        #   DAC_res = self.Sx  (input slice width; 1 bit for binary input slices)
        #   Vdd     = self.Vin
        # Number of DACs = K * N. The DAC fires once per input bit-slice → x Lx.
        # Sanity: 50 fF * 1 bit * (0.4 V)^2 = 8 fJ per conversion.
        self.K7_fF = 50.0  # fF (switched cap per unit DAC resolution)
    ...
    def evaluate_configuration(self, K, r, Sw, M, N, Lx, g):
        ...
        # --- Edac per conversion (one DAC, one input bit-slice) ---
        Edac_per_conv_fJ = self.K7_fF * self.Sx * (self.Vin**2)

        # --- Total energy for one MVM ---
        total_Ecap_fJ = Ecap_per_group_J * K_eff * 1e15 * num_adcs
        total_Eadc_fJ = Eadc_per_conv_fJ * num_adcs * Lx

        # DAC energy: (K * N) DACs, each firing once per input bit-slice (x Lx)
        num_dacs = K * N
        ...
        Edac_fJ_bOP = total_Edac_fJ / total_bit_ops
        ...
        return {
            ...
            'g': g, 'Pillars': num_pillars, 'ADCs': num_adcs, 'DACs': num_dacs,
            ...
            'Edac(fJ/bOP)': round(Edac_fJ_bOP, 4),
    ...
    def run_design_space_exploration(self, tech, K_range, r_range, Sw_range, M_range, N_range, Lx_range, g_range):
        ...
        if not M_range: M_range = list(range(128, 512, 128))
        if not N_range: N_range = list(range(128, 512, 128))
...
if __name__ == "__main__":
    ...
    if len(df) > 0:
        display_cols = [
            'K', 'g', 'K_eff', 'ADCs', 'DACs', 'Cols(M)', 'Rows(N)', 'ON/OFF', 'Sw', 'Lx',
            'Latency(ns)', 'TbOPS', 'MAC TOPS', 'Total E(pJ)',
            'Ecap(fJ/bOP)', 'Eadc(fJ/bOP)', 'Edac(fJ/bOP)', 'Etot(fJ/bOP)',
