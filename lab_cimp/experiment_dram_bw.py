...
print("Starting DRAM Bandwidth & SRAM Size 2D Experiment...")
print("-" * 75)
print(f"{'SRAM Size':<10} | {'DRAM BW':<12} | {'Latency (Cycles)':<16} | {'Energy (pJ)'}")
print("-" * 75)
...
for size_kb in sizes_kb:
    ...
    for bw in bandwidths:
        ...
        temp_yaml = f"lab_cimp/inputs/hardware/temp_cimp_drambw_{bw}_sram_{size_kb}KB.yaml"
        ...
        try:
            energy, latency, tclk, area, zigzag_results = get_hardware_performance_zigzag(
                ...
                opt="latency", 
                dump_folder=f"lab_cimp/outputs/exp_drambw_{bw}_sram_{size_kb}KB",
...
if results:
    ...
    if has_data:
        ...
        ax2.legend(title="SRAM Size")

        plt.tight_layout()
        plot_path = "figures/dram_2d_sweep.png"
