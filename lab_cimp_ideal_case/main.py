...
topsmm2_system = tops_system / cmes[0].area_total if cmes[0].area_total > 0 else 0

# Calculate macro-level performance metrics
imc_macro = cmes[0].accelerator.operational_array

# T-bOPS (Tera bit operations per second) like cimp_analyser.py
Lx = imc_macro.activation_precision / imc_macro.bit_serial_precision
Sw = imc_macro.weight_precision
tbops_system = (total_mac_count * Lx * Lx * Sw) / delay_in_ns / 1000
...
system_performance_info = f"MAC TOPS: {round(tops_system, 5)}, T-bOPS: {round(tbops_system, 3)}, TOP/s/W: {round(topsw_system, 7)}, \
...
# CIMP Analyser Direct Comparison metrics
mac_latency_ns = imc_macro.tclk
total_energy_pJ = sum([v for v in imc_macro.get_peak_energy_single_cycle().values()])
total_bit_ops = imc_macro.wordline_dim_size * imc_macro.bitline_dim_size * (imc_macro.activation_precision / imc_macro.bit_serial_precision) * imc_macro.weight_precision * imc_macro.nb_of_banks
etot_fJ_bOP = (total_energy_pJ * 1000) / total_bit_ops if total_bit_ops > 0 else 0

logger.info("=== CIMP Analyser Verification ===")
logger.info("EDP: %.2f", etot_fJ_bOP * mac_latency_ns)
logger.info("TOPS (MAC): %.3f", tops_peak)
logger.info("TOPS/W (MAC): %.3f", topsw_peak)
logger.info("POPS/W/b: %.1f", 1.0 / etot_fJ_bOP if etot_fJ_bOP > 0 else 0)
logger.info("----------------------------------")
logger.info("Latency(ns): %.1f", mac_latency_ns)
logger.info("T-put(TbOPS): %.3f", tops_peak * 32)
logger.info("Total E(pJ): %.2f", total_energy_pJ)
logger.info("Etot(fJ/bOP): %.3f", etot_fJ_bOP)
