...
class CostModelEvaluationForIMC(CostModelEvaluation):
    ...
    * mapping: The combined spatial and temporal mapping object where access patterns are computed.
    ...
    def calc_latency(self):
        ...
        # --- CIMP FIX: Remove artificial spatial penalty for preloaded weights ---
        if getattr(self.operational_array, "weights_preloaded", False):
            # Calculate the onloading critical path dynamically for all non-weight inputs
            max_onloading_cycle = 0
            for op in self.layer.input_operands:
                mem_op = self.memory_operand_links.layer_to_mem_op(op)
                if mem_op.name == "I2":  # Skip the preloaded weights memory operand
                    continue
                op_onloading = (
                    self.data_loading_individual_part.get(op, 0) + 
                    self.data_loading_half_shared_part.get(op, 0) + 
                    self.data_loading_shared_part.get(op, 0)
                )
                max_onloading_cycle = max(max_onloading_cycle, op_onloading)
            
            self.data_onloading_cycle = max_onloading_cycle
