...
class AcceleratorFactory:
    ...
    def create(self, core_id: int = 0, shared_mem_group_id: int | None = None) -> Accelerator:
        ...
        for mem_name in self.data["memories"]:
            memory_data = self.data["memories"][mem_name]
            
            # --- CIMP Override Logic ---
            if mem_name == "cells_cimp" and hasattr(operational_array, "is_cimp") and operational_array.is_cimp:
                # Override the generic YAML w_cost with true CIMP physics
                memory_data["w_cost"] = operational_array.get_cimp_weight_programming_cost()
                
                # If weights are preloaded, eliminate latency by making bandwidth infinite
                if getattr(operational_array, "weights_preloaded", False):
                    for port in memory_data.get("ports", []):
                        port["bandwidth_min"] = 999999999
                        port["bandwidth_max"] = 999999999
            # ---------------------------

            memory_factory = MemoryFactory(
                mem_name, memory_data, shared_mem_group_id=shared_mem_group_id
            )
    ...
    def create_imc_array(self) -> ImcArray:
        ...
        cimp_adc_grouping = op_array_data.get("cimp_adc_grouping", 1)
        oa_dims: list[str] = op_array_data["dimensions"]
        dimension_sizes: dict[OADimension, int] = {
            OADimension(oa_dim): op_array_data["sizes"][i] for i, oa_dim in enumerate(oa_dims)
        }
        ...
        # From IMC cells
        cells_data: dict[str, Any] = self.data["memories"].get("cells") or self.data["memories"].get("cells_cimp")
        cells_size = cells_data["size"]
        cells_area = cells_data["area"]
        auto_cost_extraction = cells_data.get("auto_cost_extraction", False)
        
        weights_preloaded = op_array_data.get("weights_preloaded", False)
