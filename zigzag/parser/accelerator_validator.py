...
class AcceleratorValidator:
    ...
    def validate_cells_imc(self):
        if "cells" not in self.data["memories"] and "cells_cimp" not in self.data["memories"]:
            self.invalidate("IMC architecture has no memory level called `cells` or `cells_cimp`.")

        cells_data = self.data["memories"].get("cells") or self.data["memories"].get("cells_cimp")
