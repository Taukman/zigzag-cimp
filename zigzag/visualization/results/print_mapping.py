...
def print_mapping(cme: CostModelEvaluationABC, offsets: int = 2):
    ...
    loop_column_width = (
        max([len(x[0].name) for x in temporal_loops], default=0) + 14 + offsets * (len(temporal_loops) + len(spatial_loops)) + 5
    )
    memory_column_width = max([len(i) for i in memories], default=0) + 5
